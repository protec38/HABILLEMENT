const App = {
    ...
    box.innerHTML=`<div class="card"><h1>Bénévoles</h1>
      ...
      <table class="table"><thead><tr><th>Nom</th><th>Prénom</th><th>Notes</th><th></th></tr></thead><tbody>
        ${data.map(v=>`<tr>
          ...
          <td><div class="chips"><button class="btn btn-ghost" onclick='App.editVol(${v.id}, ${JSON.stringify(v).replaceAll("'","&apos;")})'>Modifier</button><button class="btn btn-ghost" onclick='App.viewVol(${v.id}, ${JSON.stringify(v).replaceAll("'","&apos;")})'>Voir</button></div></td>
        </tr>`).join('')}
      </tbody></table></div>`; },
  ...
  editVol(id,v){ this.openModal('Modifier bénévole', `<div class="grid-3"><input id="e_first" class="input" value="${v.first_name}"><input id="e_last" class="input" value="${v.last_name}"><input id="e_note" class="input" value="${v.note||''}"></div><div class="toolbar" style="justify-content:flex-end;"><button class="btn btn-primary" onclick="App.saveEditVol(${id})">Enregistrer</button></div>`); },
  ...
  async viewVol(id,v){ const loans=await this.fetchJSON(`/api/volunteers/${id}/loans`);
    const html = `<div class="grid-2"><div><div class="muted">Nom</div><div class="badge">${v.last_name}</div></div><div><div class="muted">Prénom</div><div class="badge">${v.first_name}</div></div></div>
      ...
      <div class="mt"><h3>Prêts en cours</h3>${loans.length?`<table class="table"><thead><tr><th>Article</th><th>Qté</th><th>Depuis</th></tr></thead><tbody>${loans.map(l=>`<tr><td>${l.type} / ${l.size||'—'}</td><td>${l.qty}</td><td>${new Date(l.since).toLocaleString()}</td></tr>`).join('')}</tbody></table>`:'<p class="muted">Aucun prêt en cours</p>'}</div>`;
  ...
  async addUser(){ const name=this.qs('#u_name').value.trim(), email=this.qs('#u_email').value.trim(), password=this.qs('#u_pass').value; if(!name||!email||!password) return this.flash('Tous les champs sont requis', false);
    await this.fetchJSON('/api/users',{method:'POST', body: JSON.stringify({name,email,password,role:'admin'})}); this.renderAdmin(); this.flash('Compte admin créé'); },
  ...
  async ppSearch(){ const fn=this.qs('#pp_first').value.trim(); const ln=this.qs('#pp_last').value.trim(); const err=this.qs('#pp_error'); err.textContent='';
    try{ const v=await this.fetchJSON(`/api/public/volunteer?first_name=${encodeURIComponent(fn)}&last_name=${encodeURIComponent(ln)}`); this.ppVolunteer=v; this.qs('#pp_step1').classList.add('hidden'); this.qs('#pp_step2').classList.remove('hidden'); this.qs('#pp_vol').textContent=`Bénévole : ${v.last_name} ${v.first_name}`; const antennaParam=this.publicAntennaId?`?antenna_id=${this.publicAntennaId}`:''; const stock=await this.fetchJSON('/api/public/stock'+antennaParam); const sel=this.qs('#pp_item'); sel.innerHTML=stock.map(s=>`<option value="${s.id}">#${s.id} – ${s.type} / ${s.size||'—'} @ ${s.antenna} (Qté ${s.quantity})</option>`).join(''); }
