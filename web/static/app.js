...
const App = {
  ...
  async renderAdmin(){
    ...
    const ldap=await this.fetchJSON('/api/admin/ldap/config');
    const overdue=this.getSetting('overdue_days',30);
    const defThr=this.getSetting('default_threshold',5);
    el.innerHTML=`<div class="card"><div class="chips" style="justify-content:space-between"><h2>Administration</h2><div class="chips"><button class="btn btn-ghost" onclick="App.viewLogs()">Journaux</button><button class="btn btn-primary" onclick="App.modalAddUser()">+ Utilisateur</button></div></div><div class="grid-3 mt"><div><label class="muted">Jours avant retard</label><input id="set_overdue" class="input" type="number" min="1" value="${overdue}" onblur="App.saveAdminSettings()"></div><div><label class="muted">Seuil stock bas par défaut</label><input id="set_threshold" class="input" type="number" min="0" value="${defThr}" onblur="App.saveAdminSettings()"></div><div class="muted" style="display:flex;align-items:flex-end">Réglages locaux appliqués immédiatement.</div></div><h3 class="mt">LDAP</h3><div class="grid-3"><div><label class="muted">LDAP_BIND_PASSWORD (ne pas toucher sauf si changement LLDAP)</label><input id="ldap_bind_password" class="input" type="password" placeholder="${ldap.bind_password_configured?'Configuré':'Non configuré'}"></div><div><label class="muted">Groupe requis</label><input class="input" value="${this.esc(ldap.required_group)}" disabled></div><div class="chips" style="align-items:flex-end"><button class="btn btn-primary" onclick="App.saveLdapConfig()">Enregistrer LDAP</button><button class="btn btn-ghost" onclick="App.modalLdapDiagnostic()">Diagnostic</button></div></div><p class="muted">Serveur ${this.esc(ldap.url)} · Bind ${this.esc(ldap.bind_dn)}</p><h3 class="mt">Utilisateurs</h3><table class="table"><thead><tr><th>Nom</th><th>Email</th><th>Rôle</th><th>Source</th><th></th></tr></thead><tbody>${users.map(u=>`<tr><td>${this.esc(u.name)}</td><td>${this.esc(u.email)}</td><td><span class="badge">${this.esc(u.role)}</span></td><td><span class="badge">${this.esc(u.source||'local')}</span></td><td class="chips"><button class="btn btn-ghost" onclick='App.modalEditUser(${u.id}, ${JSON.stringify(u).replaceAll("'","&apos;")})'>Modifier</button><button class="btn btn-ghost" onclick='App.deleteUser(${u.id})'>Supprimer</button></td></tr>`).join('')}</tbody></table></div>`;
  },
  ...
  async saveLdapConfig(){ const bind_password=this.qs('#ldap_bind_password').value; if(!bind_password) return this.flash('Aucun changement LDAP'); await this.fetchJSON('/api/admin/ldap/config',{method:'PUT', body: JSON.stringify({bind_password})}); this.qs('#ldap_bind_password').value=''; this.flash('Configuration LDAP enregistrée'); this.renderAdmin(); },
  modalLdapDiagnostic(){ this.openModal('Diagnostic LDAP', `<div class="grid-2"><input id="ldap_test_user" class="input" placeholder="Identifiant LDAP de test"><input id="ldap_test_pass" class="input" type="password" placeholder="Mot de passe LDAP de test"></div><div class="chips mt" style="justify-content:flex-end"><button class="btn btn-primary" onclick="App.runLdapDiagnostic()">Lancer</button></div><div id="ldap_diag_result" class="mt"></div>`); },
  async runLdapDiagnostic(){ const username=this.qs('#ldap_test_user').value.trim(); const password=this.qs('#ldap_test_pass').value; const box=this.qs('#ldap_diag_result'); box.innerHTML='<p class="muted">Diagnostic en cours...</p>'; try{ const r=await this.fetchJSON('/api/admin/ldap/diagnostic',{method:'POST', body: JSON.stringify({username,password})}); box.innerHTML=`<table class="table"><thead><tr><th>Test</th><th>Statut</th><th>Détail</th></tr></thead><tbody>${r.steps.map(s=>`<tr><td>${this.esc(s.name)}</td><td><span class="badge ${s.status==='error'?'badge-danger':s.status==='ok'?'badge-green':''}">${this.esc(s.status)}</span></td><td class="muted">${this.esc(s.detail)}</td></tr>`).join('')}</tbody></table>`; }catch(e){ box.innerHTML=`<div class="alert">${this.esc(e.message||'Diagnostic impossible')}</div>`; } },
  ...
  modalEditUser(id,u){ const ldap=(u.source||'local')==='ldap'; this.openModal('Modifier utilisateur', `<div class="grid-3"><input id="eu_name" class="input" value="${this.esc(u.name)}"><input id="eu_role" class="input" value="${this.esc(u.role)}"><input id="eu_pass" class="input" type="password" placeholder="${ldap?'Mot de passe géré dans LLDAP':'Nouveau mot de passe (optionnel)'}" ${ldap?'disabled':''}></div><div class="chips" style="justify-content:flex-end"><button class="btn btn-primary" onclick="App.saveUser(${id})">Enregistrer</button></div>`); },
