/* static/app.js — FULL with robust login + error surfacing */

(function(){
  // ===== Global error surface (affiche toute erreur JS à l'écran) =====
  const showFatal = (msg) => {
    try {
      const flash = document.getElementById("flash");
      if (flash) { flash.innerHTML = `<div class="toast">⚠️ ${msg}</div>`; }
      console.error("[FATAL]", msg);
    } catch {}
  };
  window.addEventListener("error", (e) => showFatal(e.message || "Erreur Javascript"));
  window.addEventListener("unhandledrejection", (e) => showFatal((e.reason && e.reason.message) || "Erreur Promise"));
  ...
  const App = {
    ...
    async fetchJSON(url, opts = {}) {
      ...
      let data = null;
      try { data = text ? JSON.parse(text) : null; } catch { /* HTML/texte */ }
      if (!res.ok) {
    ...
    flash(msg) {
      ...
      if (!el) { console.warn("FLASH:", msg); return; }
    ...
    renderNav() {
      const el = this.qs("#nav"); if (!el) return;
      el.innerHTML = "";
    ...
    async init() {
      console.log("App boot — JS chargé");
      // Sécurise le bouton login (binding programmatique en plus de l'attribut onclick)
      const btn = document.getElementById("loginBtn");
      if (btn) btn.addEventListener("click", (e) => { e.preventDefault(); this.login(); });

      // Affiche un avertissement si le cookie CSRF est manquant (souvent Secure=true en HTTP)
      const csrf = this.getCookie("XSRF-TOKEN");
      if (!csrf) {
        console.warn("Cookie XSRF-TOKEN absent. En dev HTTP, assure-toi que SESSION_COOKIE_SECURE=false côté serveur.");
        const warn = document.createElement("div");
        warn.className = "alert";
        warn.innerHTML = "ℹ️ Cookie CSRF absent. Si vous êtes en HTTP (dev), mettez <code>SESSION_COOKIE_SECURE=false</code> côté serveur pour que le cookie puisse être posé.";
        const loginView = document.getElementById("loginView");
        if (loginView) loginView.insertBefore(warn, loginView.firstChild);
      }

      const m = location.pathname.match(/^\/a\/(\d+)/);
      if (m) this.publicAntennaId = Number(m[1]);
      if (this.publicAntennaId) { // page publique
        const lv = document.getElementById("loginView"); if (lv) lv.classList.add("hidden");
        this.renderNav(); this.show("pretPublic"); return;
      }
      try {
        const me = await this.fetchJSON("/api/me");
        if (me.ok) { this.user = me.user; this.renderNav(); const lv = this.qs("#loginView"); if (lv) lv.classList.add("hidden"); this.show("dashboard"); return; }
      } catch (e) {
        console.log("ME not authenticated yet:", e.message);
      }
      this.renderNav(); const lv = this.qs("#loginView"); if (lv) lv.classList.remove("hidden");
    },
    ...
    async login() {
      ...
      if (!email || !password) { if (err) { err.textContent = "Email et mot de passe requis"; err.classList.remove("hidden"); } this.flash("Champs requis"); return; }
      if (btn) { btn.disabled = true; btn.dataset.old = btn.textContent; btn.textContent = "Connexion..."; }
      try {
        ...
        this.flash("Connexion refusée : " + msg);
      } finally {
        if (btn) { btn.disabled = false; btn.textContent = btn.dataset.old || "Se connecter"; }
      }
    },
    ...
    async renderAntennes(){
      const el=this.qs('#antennes'); if (!el) return;
    ...
    /* ------------------------------ Stock ------------------------------ */
    async renderStock(){
      const el=this.qs('#stock'); if (!el) return;
    ...
    async renderBenevoles(){
      const el=this.qs('#benevoles'); if (!el) return;
      el.innerHTML=`<div class="card">
        <div class="chips" style="justify-content:space-between">
          <h2>Bénévoles</h2>
          <div class="chips">
    ...
    /* ------------------------------ Prêts ------------------------------ */
    async renderPrets(){
      const el=this.qs('#prets'); if (!el) return;
    ...
    async renderInventaire(){
      const ants=await this.fetchJSON('/api/antennas').catch(()=>[]);
      const el=this.qs('#inventaire'); if (!el) return;
    ...
    /* ------------------------------ Admin ------------------------------ */
    async renderAdmin(){
      const el=this.qs('#admin'); if (!el) return;
    ...
    async renderPretPublic(){
      const el=this.qs('#pretPublic'); if (!el) return;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    // Bind de secours si l'attribut onclick n'est pas pris en compte
    const btn = document.getElementById("loginBtn");
    if (btn) btn.addEventListener("click", (e) => { e.preventDefault(); App.login(); });

    // Raccourci ENTER pour le login + init app
