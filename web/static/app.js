const App = {
  ...
  flash(msg, ok=true){ const el=document.getElementById('flash'); el.innerHTML=`<div class="toast">${msg}</div>`; setTimeout(()=>el.innerHTML='',2500); },
  ...
  async renderDashboard(){
    ...
    el.innerHTML=`<div class="card"><h2>Statistiques</h2>
      ...
      <div class="chips mt">
        ...
        <a class="btn btn-ghost" href="/api/export/loans.csv">⬇️ Export prêts</a>
      </div>
    </div>`;
  },
  ...
  async renderAntennes(){
    ...
    el.innerHTML=`<div class="card">
      ...
      <table class="table"><thead><tr><th>Nom</th><th>Adresse</th><th></th></tr></thead><tbody>
        ${ants.map(a=>`<tr>
          <td>${a.name}</td><td class="muted">${a.address||''}</td>
          <td class="chips">
  ...
  modalAddAntenna(){ this.openModal('Nouvelle antenne', `
    <div class="grid-2"><input id="ant_name" class="input" placeholder="Nom"><input id="ant_addr" class="input" placeholder="Adresse"></div>
  ...
  async saveAntenna(){ const name=this.qs('#ant_name').value.trim(); const address=this.qs('#ant_addr').value.trim();
  ...
  modalEditAntenna(id,a){ this.openModal('Modifier antenne', `
    <div class="grid-2"><input id="e_ant_name" class="input" value="${a.name}"><input id="e_ant_addr" class="input" value="${a.address||''}"></div>
  ...
  async updateAntenna(id){ const name=this.qs('#e_ant_name').value.trim(); const address=this.qs('#e_ant_addr').value.trim();
    await this.fetchJSON('/api/antennas/'+id,{method:'PUT', body: JSON.stringify({name,address})}); this.closeModal(); this.renderAntennes(); this.flash('Antenne mise à jour'); },
  async deleteAntenna(id){ if(!confirm('Supprimer cette antenne ?')) return; try{
      await this.fetchJSON('/api/antennas/'+id,{method:'DELETE'}); this.renderAntennes(); this.flash('Antenne supprimée');
    }catch(e){ this.flash(e.message||'Suppression impossible',false); } },

  // ---------- Stock (CRUD + selects Types/Antennes) ----------
  async renderStock(){
    const el=this.qs('#stock');
    const [types, ants, stock]=await Promise.all([
      this.fetchJSON('/api/types'),
      this.fetchJSON('/api/antennas'),
      this.fetchJSON('/api/stock')
    ]);

    const optType = (val)=>['<option value="">Choisir un type</option>'].concat(types.map(t=>`<option value="${t.id}" ${val==t.id?'selected':''}>${t.label}</option>`)).join('');
    const optAnt  = (val)=>['<option value="">Choisir une antenne</option>'].concat(ants.map(a=>`<option value="${a.id}" ${val==a.id?'selected':''}>${a.name}</option>`)).join('');
    ...
    el.innerHTML=`<div class="card">
      ...
      <table class="table"><thead><tr><th>Type</th><th>Taille</th><th>Antenne</th><th>Qté</th><th></th></tr></thead><tbody>
        ${stock.map(s=>`<tr>
          <td>${s.garment_type}</td><td>${s.size||'—'}</td><td>${s.antenna}</td><td>${s.quantity}</td>
          <td class="chips">
            <button class="btn btn-ghost" onclick='App.modalEditStock(${s.id}, ${JSON.stringify({id:s.id, type_id:s.garment_type_id, ant_id:s.antenna_id, size:s.size||"", qty:s.quantity}).replaceAll("'","&apos;")})'>Modifier</button>
    ...
    // enregistreurs pour usage dans modales
    this._types = types; this._ants = ants; this._optType = optType; this._optAnt = optAnt;
  },
  ...
  async saveType(){ const label=this.qs('#new_type').value.trim(); const has_size=this.qs('#new_has_size').checked;
    if(!label) return this.flash('Libellé requis',false);
    await this.fetchJSON('/api/types',{method:'POST', body: JSON.stringify({label,has_size})}); this.closeModal(); this.renderStock(); this.flash('Type ajouté'); },
  ...
  async saveStock(){ const t=Number(this.qs('#s_type').value); const a=Number(this.qs('#s_ant').value); const size=this.qs('#s_size').value.trim()||null; const qty=Number(this.qs('#s_qty').value||0);
    if(!t||!a||qty<=0) return this.flash('Type, antenne et quantité requis',false);
    await this.fetchJSON('/api/stock',{method:'POST', body: JSON.stringify({garment_type_id:t, antenna_id:a, size, quantity:qty})});
    this.closeModal(); this.renderStock(); this.flash('Stock ajouté'); },
  ...
  async deleteStock(id){ if(!confirm('Supprimer cet article ?')) return; await this.fetchJSON('/api/stock/'+id,{method:'DELETE'}); this.renderStock(); this.flash('Article supprimé'); },
  ...
  async addUser(){ const name=this.qs('#u_name').value.trim(), email=this.qs('#u_email').value.trim(), password=this.qs('#u_pass').value;
    ...
    try{ await this.fetchJSON('/api/users',{method:'POST', body: JSON.stringify({name,email,password,role:'admin'})}); this.closeModal(); this.renderAdmin(); this.flash('Compte admin créé'); }
  ...
  async saveUser(id){ const name=this.qs('#eu_name').value.trim(), role=this.qs('#eu_role').value.trim(), password=this.qs('#eu_pass').value;
    await this.fetchJSON('/api/users/'+id,{method:'PUT', body: JSON.stringify({name,role,password})}); this.closeModal(); this.renderAdmin(); this.flash('Compte mis à jour'); },
