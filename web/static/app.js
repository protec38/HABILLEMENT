const App = {
  ...
  async init(){
    ...
    // URL publique : masquer le login et ne montrer que l'écran prêt public
  ...
  // ------------- Dashboard + Analytics + Carte + Alertes -------------
  async renderDashboard(){
    ...
    // Alertes stock bas par antenne (seuil sur antenne.low_stock_threshold, sinon fallback 5)
    ...
    // Groupes pour analytics de base
    const byAntenna = {};
    const byType = {};
    stock.forEach(s=>{
    ...
    // Tentative d'historique (si l’API existe), sinon on bascule en “open loans only”
    let hist = [];
    try {
      const h = await this.fetchJSON('/api/analytics/history'); // <- endpoint optionnel
      hist = Array.isArray(h)? h: [];
    } catch {
      // fallback minimal: on fabrique un historique synthétique à partir des prêts ouverts (peu précis)
      hist = openLoans.map(l=>({date:l.since, type:l.type, antenna:l.antenna, qty:l.qty}));
    }

    // Top 10 des articles les plus prêtés (sur hist)
    const lendCountByType = {};
    (hist||[]).forEach(x=>{
      const k=x.type||'?'; lendCountByType[k]=(lendCountByType[k]||0)+(x.qty||1);
    });
    const top10 = Object.entries(lendCountByType).sort((a,b)=>b[1]-a[1]).slice(0,10);

    // Saison (12 barres mois-1 -> mois-12)
    const perMonth = new Array(12).fill(0);
    (hist||[]).forEach(x=>{
      const d=new Date(x.date||Date.now()); const idx = (new Date().getMonth() - d.getMonth() + 12)%12;
      perMonth[idx] += (x.qty||1);
    });
    const monthLabels = Array.from({length:12},(_,i)=> {
      const d=new Date(); d.setMonth(d.getMonth()-i); return d.toLocaleString(undefined,{month:'short'});
    });
      ...
      <div class="card">
        ...
        <div class="grid-2 mt">
          <div>
            ...
            ${lowStock.length? `<table class="table">
              <thead><tr><th>Type</th><th>Taille</th><th>Antenne</th><th>Qté</th></tr></thead>
              <tbody>${lowStock.map(s=>`<tr><td>${s.garment_type}</td><td>${s.size||'—'}</td><td>${s.antenna}</td><td><span class="badge">${s.quantity}</span></td></tr>`).join('')}</tbody>
            </table>` : `<p class="muted">Aucune alerte.</p>`}
          </div>
          <div>
            ...
            ${overdue.length? `<table class="table">
              <thead><tr><th>Bénévole</th><th>Article</th><th>Jours</th><th></th></tr></thead>
              <tbody>${overdue.map(l=>`<tr><td>${l.volunteer}</td><td>${l.type} ${l.size||''}</td><td><span class="badge" style="background:rgba(239,68,68,.15);border-color:rgba(239,68,68,.35);color:#fecaca">${l.days}</span></td><td><button class="btn btn-ghost" onclick="App.returnLoan(${l.id})">Rendu</button></td></tr>`).join('')}</tbody>
            </table>` : `<p class="muted">Aucun retard.</p>`}
          </div>
        </div>
        ...
        <div class="grid-2 mt">
          <div class="card">
            ...
            ${this.svgBar(Object.entries(byAntenna), 320, 160)}
          </div>
          <div class="card">
            <h3>Stock par type</h3>
            ${this.svgBar(Object.entries(byType), 320, 160)}
          </div>
        </div>

        <div class="grid-2 mt">
          <div class="card">
            <h3>Top 10 le plus prêté</h3>
            ${this.svgBar(top10, 320, 160)}
          </div>
          <div class="card">
            <h3>Saisonnalité (12 derniers mois)</h3>
            ${this.svgBar(monthLabels.map((m,i)=>[m, perMonth[i]||0]).reverse(), 320, 160)}
          </div>
        </div>
  ...
  // ------------- Mini chart SVG (barres horizontales) -------------
  svgBar(pairs, w=320, h=160){
    const pad=20, row=18, gap=6;
    const data=(pairs||[]).slice(0,8); // compact
  ...
  // ------------- Antennes (seuil d’alerte éditable) -------------
  async renderAntennes(){
    const el=this.qs('#antennes'); const ants=await this.fetchJSON('/api/antennas');
  ...
  modalAddAntenna(){ this.openModal('Nouvelle antenne', `
    <div class="grid-3">
  ...
  async saveAntenna(){
    const name=this.qs('#ant_name').value.trim(); const address=this.qs('#ant_addr').value.trim();
  ...
  modalEditAntenna(id,a){ this.openModal('Modifier antenne', `
    <div class="grid-3">
  ...
  async updateAntenna(id){
    const name=this.qs('#e_ant_name').value.trim(); const address=this.qs('#e_ant_addr').value.trim();
  ...
  async loadStock(){
    const t=this.qs('#f_type')?.value||''; const a=this.qs('#f_ant')?.value||'';
  ...
  renderTagsInline(tags){ tags = Array.isArray(tags)? tags: String(tags||'').split(',').map(x=>x.trim()).filter(Boolean); 
    if(!tags.length) return `<span class="muted">—</span>`;
    return `<div class="chips">${tags.map(t=>`<span class="badge">${t}</span>`).join('')}</div>`;
  },
  modalAddType(){ this.openModal('Ajouter un type', `
  ...
  modalAddStock(){ this.openModal('Ajouter au stock', `
    <div class="grid-4">
  ...
  async saveStock(){
    const t=Number(this.qs('#s_type').value); const a=Number(this.qs('#s_ant').value);
    const size=this.qs('#s_size').value.trim()||null; const qty=Number(this.qs('#s_qty').value||0);
  ...
  modalEditStock(id, s){ this.openModal('Modifier un article de stock', `
    <div class="grid-4">
  ...
  // ------------- Bénévoles (identique fonctionnalités précédentes) -------------
  async renderBenevoles(){
    const el=this.qs('#benevoles'); const data=await this.fetchJSON('/api/volunteers');
    el.innerHTML=`<div class="card">
      <div class="chips" style="justify-content:space-between">
        <h2>Bénévoles</h2>
  ...
  modalAddVol(){ this.openModal('Nouveau bénévole', `
  ...
  async addVol(){ const first_name=this.qs('#v_first').value.trim(); const last_name=this.qs('#v_last').value.trim(); const note=this.qs('#v_note').value.trim();
    ...
    await this.fetchJSON('/api/volunteers',{method:'POST', body: JSON.stringify({first_name,last_name,note})}); this.closeModal(); this.renderBenevoles(); this.flash('Bénévole créé'); },
  modalEditVol(id, v){ this.openModal('Modifier bénévole', `
  ...
  async viewVol(id, v){ const loans=await this.fetchJSON(`/api/volunteers/${id}/loans`); const html = `
      <div class="grid-2">
  ...
  // ------------- Prêts en cours -------------
  async renderPrets(){
    const el=this.qs('#prets'); const r=await this.fetchJSON('/api/loans/open');
    el.innerHTML=`<div class="card"><h2>Prêts en cours</h2>
      <table class="table"><thead><tr><th>Bénévole</th><th>Article</th><th>Qté</th><th>Depuis</th><th></th></tr></thead><tbody>
        ${r.map(l=>`<tr><td>${l.volunteer}</td><td>${l.type} / ${l.size||'—'} @ ${l.antenna}</td><td>${l.qty}</td><td>${new Date(l.since).toLocaleString()}</td><td><button class="btn btn-ghost" onclick="App.returnLoan(${l.id})">Marquer rendu</button></td></tr>`).join('')}
      </tbody></table></div>`;
  },
  ...
  modalEditUser(id,u){ this.openModal('Modifier utilisateur', `
