...
login_manager.login_view = "index"  # redirigé vers la SPA (login inline)
...
# --- DB readiness (important en Docker) ---
def wait_for_db(max_tries: int = 30, delay: float = 1.0):
    """Attend que Postgres réponde à SELECT 1 avant d'initialiser le schéma."""
...
with app.app_context():
    wait_for_db()  # attend que la DB soit prête
    db.create_all()
...
def stats():
    ...
    return jsonify(
        {"stock_total": stock_total, "prets_ouverts": loans_open, "benevoles": volunteers}
    )
...
def types_add():
    ...
    db.session.add(t)
    db.session.commit()
    return jsonify({"id": t.id, "label": t.label, "has_size": t.has_size})
...
def volunteers_list():
    ...
    return jsonify(
        [{"id": v.id, "first_name": v.first_name, "last_name": v.last_name, "note": v.note} for v in items]
    )
...
if __name__ == "__main__":
    # En prod Docker, le CMD du Dockerfile lance déjà app.py
