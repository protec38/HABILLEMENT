...
db = SQLAlchemy(app)
login_manager = LoginManager(app); login_manager.login_view = "index"
...
def login_api():
    ...
    if not u or not bcrypt.verify(password, u.pwd_hash): return jsonify({"ok":False,"error":"Identifiants invalides"}),401
    login_user(u); return jsonify({"ok":True,"user":{"id":u.id,"email":u.email,"name":u.name,"role":u.role}})

@app.post("/api/logout"); @login_required
def logout_api(): logout_user(); return jsonify({"ok":True})

@app.get("/api/me")
def me():
    if current_user.is_authenticated:
        return jsonify({"ok":True,"user":{"id":current_user.id,"email":current_user.email,"name":current_user.name,"role":current_user.role}})
    return jsonify({"ok":False})

@app.get("/api/stats"); @login_required
def stats():
...
@app.get("/api/users"); @login_required
def users_list():
    users = User.query.order_by(User.email).all()
    return jsonify([{"id":u.id,"email":u.email,"name":u.name,"role":u.role} for u in users])

@app.post("/api/users"); @login_required
def users_add():
    d=request.get_json() or {}
    u=User(email=d.get("email","").strip().lower(), name=d.get("name","").strip(),
           pwd_hash=bcrypt.hash(d.get("password","admin123")), role=d.get("role","admin"))
    db.session.add(u); db.session.commit(); return jsonify({"id":u.id})

@app.put("/api/users/<int:user_id>"); @login_required
def users_update(user_id):
    d=request.get_json() or {}
    u=db.session.get(User, user_id)
    if not u: return jsonify({"ok":False}),404
    u.name=d.get("name",u.name); u.role=d.get("role",u.role)
    if d.get("password"): u.pwd_hash=bcrypt.hash(d["password"])
    db.session.commit(); return jsonify({"ok":True})

@app.get("/api/types"); @login_required
def types_list():
...
@app.post("/api/types"); @login_required
def types_add():
    d=request.get_json() or {}
    t=GarmentType(label=d.get("label","").strip(), has_size=bool(d.get("has_size",True)))
    db.session.add(t); db.session.commit(); return jsonify({"id":t.id})

@app.get("/api/stock"); @login_required
def stock_list():
    out=[]; 
...
@app.post("/api/stock"); @login_required
def stock_add():
    d=request.get_json() or {}
...
@app.get("/api/volunteers"); @login_required
def volunteers_list():
...
@app.post("/api/volunteers"); @login_required
def volunteers_add():
    d=request.get_json() or {}
...
@app.put("/api/volunteers/<int:vol_id>"); @login_required
def volunteers_update(vol_id):
    d=request.get_json() or {}
    v=db.session.get(Volunteer, vol_id)
    if not v: return jsonify({"ok":False}),404
    v.first_name=d.get("first_name",v.first_name).strip(); v.last_name=d.get("last_name",v.last_name).strip(); v.note=d.get("note",v.note).strip()
    db.session.commit(); return jsonify({"ok":True})

@app.get("/api/volunteers/<int:vol_id>/loans"); @login_required
...
@app.get("/api/loans/open"); @login_required
def loans_open():
    res=[]; 
...
@app.post("/api/loans/return/<int:loan_id>"); @login_required
def loan_return(loan_id):
    l=db.session.get(Loan, loan_id)
    if not l or l.returned_at: return jsonify({"ok":False}),404
    l.returned_at=datetime.utcnow(); item=db.session.get(StockItem, l.stock_item_id); item.quantity += l.qty
    db.session.commit(); return jsonify({"ok":True})
